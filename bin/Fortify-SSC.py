...
from mod.rules import InputRules_Fortify_SSC as Input_Fortify_SSC
from mod.rules import AnalysisRules_Fortify_SSC as Analysis_Fortify_SSC
...
if __name__ == '__main__':
    ...
    if file_path_list == []:
        Message.error_message('没有需要分析的文件，请注意只接受标准的 Zip 压缩包')
    # 解压压缩包, 获取解压路径
    ...
    if input_args[1].get('-out') in ['report','Report']:
        ...
        p2 = Process(target=output.archive_to_report, args=(Q2, Analysis_Fortify_SSC.match_rules_list, input_args[1], unarchive_path), name='Out-Process')    # input_args 数据格式： [True，字典数据]
        p1.start()
        p2.start()
