...
from rules import Fortify_WI_InputRules as WI_In_Rules
from rules import Fortify_WI_AnalysisRules as WI_Alysis_Rules
...
if __name__ == '__main__':
    ...
    # 过滤待分析的文件或压缩包
    file_abspath_dict, unarchive_path = Check.check_files(filepath, WI_In_Rules.need_files, basepath)

    # 生成多进程需要的数据
    InputRule = {}
    InputRule['match_start'] = WI_In_Rules.match_start
    InputRule['match_any'] = WI_In_Rules.match_any
    InputRule['match_end'] = WI_In_Rules.match_end
    ruleldict = {}
    ruleldict['logs'] = WI_Alysis_Rules.log_rules_list
    ruleldict['other'] = WI_Alysis_Rules.other_rule_list
    ...
    if input_argv.get('-out') in ['report','Report']:
        ...
        for p in range(Check.get_multiprocess_counts()-1):
            p = Process(target=archive_general_report, args=(Queue_Input, ruleldict, Queue_Output, WI_In_Rules.black_list))
            p.start()