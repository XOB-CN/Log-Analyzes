...
from mod.rules import InputRules_ITOM_OBM as Input_ITOM_OBM
from mod.rules import AnalysisRules_ITOM_OBM as Analysis_ITOM_OBM
...
if __name__ == '__main__':
    ...
    # 获取需要分析的文件列表
    file_path_list = ArchiveCheck.check_archive(filename, Input_ITOM_OBM.need_file_list)
    if file_path_list == []:
    ...
    if input_args[1].get('-out') in ['report','Report']:
        ...
        p2 = Process(target=output.archive_to_report, args=(Q2, Analysis_ITOM_OBM.match_rules_list, input_args[1], unarchive_path), name='Out-Process')    # input_args 数据格式： [True，字典数据]
        p1.start()
        p2.start()

        # 启动日记分析的多进程模块
        for number in range(ArchiveCheck.get_multiprocess_counts() - 1):
            number = Process(target=general.archive_general_report, args=(Q1, Analysis_ITOM_OBM.match_rules_list, Q2, Input_ITOM_OBM.black_rule_list))
            number.start()
