...
from rules import CBK_ZipAgent_InputRules as CBK_In_Rules
from rules import CBK_ZipAgent_AnalysisRules as CBK_Alysis_Rules
...
if __name__ == '__main__':
    ...
    # 过滤待分析的文件或压缩包
    file_abspath_dict, unarchive_path = Check.check_files(filepath, CBK_In_Rules.need_files, basepath)

    # 生成多进程需要的数据
    InputRule = {}
    InputRule['match_start'] = CBK_In_Rules.match_start
    InputRule['match_any'] = CBK_In_Rules.match_any
    InputRule['match_end'] = CBK_In_Rules.match_end
    ruleldict = {}
    ruleldict['logs'] = CBK_Alysis_Rules.log_rules_list
    ruleldict['other'] = CBK_Alysis_Rules.other_rule_list
    ...
    if input_argv.get('-out') in ['report','Report']:
        ...
        for p in range(Check.get_multiprocess_counts()-1):
            p = Process(target=archive_general_report, args=(Queue_Input, ruleldict, Queue_Output, CBK_In_Rules.black_list))
            p.start()