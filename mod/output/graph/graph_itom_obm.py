...
def summary_by_count_lv1(input_argv):
    """
    从 MongoDB 中获取数据, 并生成统计图表
    基于组件统计事件的个数
    """
    ...
    tmp2_data = tmp1_data['log_type']
    tmp3_data = copy.deepcopy(tmp2_data)
    full_data = tmp3_data.value_counts('log_type').nlargest(int(input_argv.get('-top', 5)))
    ...
    # 生成图表：基于时间的组件比重（柱状图）
    plt.figure(figsize=(8,4))
    plt.title('Time Base Component_Lv1 Event Percentage')
    plt.bar(full_data.index, full_data)
    plt.xlabel(date_limit)
    plt.show()

def summary_by_count_lv2(input_argv):
    """
    从 MongoDB 中获取数据, 并生成统计图表/信息
    基于组件统计事件的个数
    """
    # 初始化参数
    db_name = input_argv.get('-db_name', None)
    ...
    tmp2_data = tmp1_data['log_type'] + '.' + tmp1_data['log_comp1']
    tmp3_data = copy.deepcopy(tmp2_data)
    full_data = tmp3_data.value_counts('log_type').nlargest(int(input_argv.get('-top', 10)))
    ...
    gui = input_argv.get('-gui')
    if gui in ['on', 'On', 'enable', 'open']:
        # 生成图表：基于时间的组件比重（柱状图）
        plt.figure(figsize=(8,4))
        plt.title('Time Base Component_Lv2 Event Percentage')
        plt.bar(full_data.index, full_data)
        plt.xlabel(date_limit)
        plt.show()
    else:
...
def summary_by_count_lv3(input_argv):
    """
    从 MongoDB 中获取数据, 并生成统计图表/信息
    基于组件统计事件的个数
    """
    ...
    tmp2_data = tmp1_data['log_type'] + '.' + tmp1_data['log_comp1'] + '.' + tmp1_data['log_comp2']
    tmp3_data = copy.deepcopy(tmp2_data)
    full_data = tmp3_data.value_counts('log_type').nlargest(int(input_argv.get('-top', 15)))
    ...
    if gui in ['on', 'On', 'enable', 'open']:
        # 生成图表：基于时间的组件比重（柱状图）
        plt.figure(figsize=(8,4))
        plt.title('Time Base Component_Lv3 Event Percentage')
        plt.bar(full_data.index, full_data)
        plt.xlabel(date_limit)
        plt.show()
    else:
